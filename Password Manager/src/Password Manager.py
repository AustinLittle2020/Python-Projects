...
# Asks for user password and reiterates if incorrect password is entered
connect = input("What is your password?\n")

# Checks if user entered password to allow access into password manager
while connect != ADMIN_PASSWORD:
    connect = input("Please enter the correct password.\n")
    if connect == "q":
        break

# Connects to sql db for password manager
conn = sqlite3.connect('pass_manager.db')

# Below are the functions to store, create, encode and retrieve passwords
...
# Checks if user already has a safe in the sql db; if they do not it creates a new one
if connect == ADMIN_PASSWORD:
    try:
        conn.execute('''CREATE TABLE KEYS
            (PASS_KEY TEXT PRIMARY KEY NOT NULL);''')
        print("Your safe was created!\nWhat would you like to store in it today?")
    except:
    ...
    while True:
        ...
        # Processes user command and reacts accordingly
        if input_ == "q":
            break
        if input_ == "sp":
