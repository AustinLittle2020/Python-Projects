import tweepy
from Tkinter import * #Allows us to build GUI interface

#Defines variables to hold personal twitter keys
consumer_key = 'consumer_key'
consumer_secret = 'consumer_secret'
access_token = 'access_token'
access_token_secret = 'access_token_secret'

#Takes personal twitter access token and links it to tweepy for automation
...
#Code that makes the bot functional
def mainFunction():
    getE1()
    search = getE1()
    ...
    if favorite == 'yes':
        for tweet in tweepy.Cursor(api.search, search).items(numberOfTweets):
            try:
                #Retweet
                tweet.favorite()
                print('Favorited tweet successfully!')
